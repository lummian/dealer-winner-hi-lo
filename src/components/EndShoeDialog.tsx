...
interface EndShoeDialogProps {
  unitSize: number;
  initialStartBalance: number | null;
  onConfirm: (startBalance: number, endBalance: number) => void;
  onSkip: () => void;
  onCancel: () => void;
}
...
export function EndShoeDialog({
  ...
  const isFirst = initialStartBalance === null;
  const [startStr, setStartStr] = useState(
    initialStartBalance !== null ? String(initialStartBalance) : ''
  );
  const [endStr, setEndStr] = useState('');

  useEffect(() => {
    if (initialStartBalance !== null) setStartStr(String(initialStartBalance));
  }, [initialStartBalance]);
  ...
  const canConfirm = startValid && endValid && unitSize > 0;

  const netMoney = canConfirm ? end - start : 0;
      ...
      <div className="w-full max-w-sm bg-slate-900 rounded-2xl border border-slate-800 p-4 space-y-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {isFirst
              ? 'Primer registro: ¿con cuánto arrancaste y cuánto tenés ahora?'
              : 'Inicial = balance al final del zapato anterior. Anotá el balance actual.'}
          </p>
        </div>
        ...
        <label className="block">
          ...
          <input
            ...
            disabled={!isFirst}
            placeholder="0"
            className="w-full mt-1 px-3 py-2 rounded-lg bg-slate-800 text-white text-lg tabular-nums border border-slate-700 disabled:opacity-60 focus:outline-none focus:border-emerald-500"
          />
        </label>
        ...
        <div className="bg-slate-800 rounded-lg p-2.5 text-center">
          ...
          {canConfirm ? (
            ...
            <div className="text-sm text-slate-500">Ingresá los dos balances</div>
          )}
        ...
        <div className="grid grid-cols-3 gap-2 pt-1">
          ...
          <button
            onClick={() => {
              if (canConfirm) onConfirm(start, end);
            }}
            disabled={!canConfirm}
        ...
        <p className="text-[10px] text-slate-500 leading-snug">
          <strong>Saltar</strong>: reinicia el zapato sin registrar (útil si jugaste sin trackear o
          no querés guardar este).
        </p>
      </div>
    </div>
  );
